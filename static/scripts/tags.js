class Note extends HTMLElement {
  constructor() {
    ...
    this.attachShadow({ mode: 'open' }); // создаем теневое дерево
  }

  connectedCallback() {
    // Вызывается, когда элемент добавляется в DOM
    const title = this.getAttribute('title'); // получаем атрибут name
    this.render(title); // вызываем метод для отображения
  }
...
class Footnote extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const text = this.getAttribute('text') // получаем атрибут name
    this.render(text); // вызываем метод для отображения
  }
...
customElements.define('footnote-t', Note);
customElements.define('footnote-c', Footnote);